...
-- 1. Update category check constraint to include 'milestone'
ALTER TABLE achievements DROP CONSTRAINT IF EXISTS achievements_category_check;
ALTER TABLE achievements ADD CONSTRAINT achievements_category_check CHECK (category IN ('streak', 'completion', 'volume', 'special', 'milestone'));

-- 2. Add new columns to achievements table
ALTER TABLE achievements ADD COLUMN IF NOT EXISTS hint_level TEXT DEFAULT 'full' CHECK (hint_level IN ('full', 'cryptic', 'hidden'));
ALTER TABLE achievements ADD COLUMN IF NOT EXISTS badge_type TEXT DEFAULT 'permanent' CHECK (badge_type IN ('permanent', 'monthly', 'seasonal', 'event'));
ALTER TABLE achievements ADD COLUMN IF NOT EXISTS is_repeatable BOOLEAN DEFAULT false;
ALTER TABLE achievements ADD COLUMN IF NOT EXISTS repeat_xp_multiplier DECIMAL(3,2) DEFAULT 1.0 CHECK (repeat_xp_multiplier > 0 AND repeat_xp_multiplier <= 1.0);
ALTER TABLE achievements ADD COLUMN IF NOT EXISTS active_from TIMESTAMPTZ;
ALTER TABLE achievements ADD COLUMN IF NOT EXISTS active_until TIMESTAMPTZ;
ALTER TABLE achievements ADD COLUMN IF NOT EXISTS is_featured BOOLEAN DEFAULT false;

-- 3. Create achievement_progress table for tracking user progress
...
-- 4. Create achievement_unlock_history for repeatable badges
...
-- 6. Update existing badges with hint levels
...
-- 7. Rename existing badges to motivational tone (remove icon prefixes from titles)
...
-- 9. Insert new intermediate badges
INSERT INTO achievements (key, title, description, icon, category, xp_reward, unlock_condition, display_order, hint_level, badge_type) VALUES
...
-- 10. Insert monthly challenge badges (repeatable)
