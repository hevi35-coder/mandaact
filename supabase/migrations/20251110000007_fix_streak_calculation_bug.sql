-- Fix Critical Bug: user_gamification table does not exist
-- Replace with proper streak calculation from check_history
...
BEGIN
  ...
  CASE v_condition_type
    ...
    -- Streak - FIXED: Calculate from check_history instead of user_gamification
    WHEN 'streak' THEN
    ...
    -- NEW: Perfect month count (milestone badge)
    ...
    -- NEW: Midnight checks (secret badge)
    ...
    -- NEW: Balanced mandalart week (secret badge)
    ...
    -- NEW: Time range checks (secret badge)
...
COMMENT ON FUNCTION evaluate_badge_progress IS '[FIXED] Calculate real-time progress for badge unlock conditions. Streak now calculated from check_history. Supports types: total_checks, streak, monthly_completion, monthly_streak, perfect_week_in_month, perfect_month_count, midnight_checks, balanced_mandalart_week, time_range_checks';
