-- Step 2: Create Badge Validation Logs Table (Safe Version)
...
-- Add RLS policies (using CREATE OR REPLACE to avoid DROP warnings)
CREATE POLICY IF NOT EXISTS "Users can view their own validation logs"
  ON badge_validation_logs FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "System can insert validation logs"
