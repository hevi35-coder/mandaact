/**
 * Badge Hint System for Mobile
 * Provides unlock condition formatting and progress messages
 * Aligned with web implementation
 */
...
export function formatUnlockCondition(
  ...
  switch (condition.type) {
    ...
    case 'monthly_completion':
      return `현재 달의 달성률이 ${condition.threshold}% 이상`
 ...
 */
export function getProgressMessage(current: number, target: number): string {
  ...
  if (percentage >= 25) {
    return `좋아요! ${current}/${target} 달성 중`
  }

  return `시작이 반입니다! ${current}/${target}`
}
