...
| 액션 관리 | ✅ 완료 | CRUD, 타입 시스템 (루틴/미션/참고), 자동 타입 추천(규칙 기반), 인라인 편집 |
...
#### 10.3.2 결제/리텐션/스토어 운영 고도화 🔄

> **목표**: “결제 신뢰”와 “리텐션/전환”을 동시에 끌어올리는 운영 체계를 만든다.
> - 결제 이슈는 “재현 가능한 로그 + UX 정책”으로 관리
> - 리텐션/전환은 “계측 기반”으로 개선

##### A) 결제 플로우 관측/안정화 (RevenueCat/StoreKit)
- [x] (모바일) 결제 실패를 단계별(stage) 로깅/분류 기반으로 정리(telemetry 스키마 추가)
- [ ] (모바일) 월간 구독 구매 실패(`code: 8`)를 단계별로 분해해 로깅/분류 (실기기 재현 필요)
  - 계정 상태(Apple ID/샌드박스), 스토어 상태(StoreKit), 상품 상태(Products/Offerings), 네트워크 상태, RevenueCat 응답
  - Sentry/PostHog 이벤트로 동일한 분류 체계 유지(“실패 원인별” 대시보드 가능하도록)
- [ ] (모바일) 결제 UX 정책 정리: fallback/재시도/복원(restore) 시나리오를 화면 상태로 명확히 노출
  - “무반응”이 아닌 “진행 중/실패/권장 조치”가 보이도록
- [ ] (모바일) App Store 계정 변경 후 비밀번호 반복 요청 UX 점검/개선
  - 불필요한 `sync/restore` 호출 빈도/타이밍 점검 및 최소화

##### B) 리텐션/전환(구독) 개선: 계측 → 가설 → 실험
- [ ] (공통) 리텐션 루프 계측 정리(온보딩→첫 만다라트 생성→첫 체크→주간 리포트→재방문)
  - 이탈 구간/시간(24h/7d) 기준으로 핵심 이벤트 정의 및 PostHog 대시보드 구축
- [ ] (공통) 무료→유료 전환 트리거 단순화(1~2개로 고정) + 노출 정책 정의
  - 예: “리포트 생성”/“광고 제거” 중 하나를 핵심 트리거로 고정하고 메시지/배치를 통일

##### C) UX 카피/랜딩(5초 전달력)
- [ ] (모바일) 마이크로카피 통일(권한/결제/실패/복원/계정삭제): “불안 감소” 톤으로 일관화
  - 용어/버튼 라벨/오류 메시지/권장 행동(다음 단계) 표준 템플릿 정의
- [ ] (웹) 랜딩 페이지 고도화: “왜 필요한가/무엇이 달라지는가”를 5초 내 전달
  - 서비스 가치 제안(1문장) + 주요 기능(3개) + 신뢰 요소(리뷰/지표/정책) + 명확한 CTA

##### D) ASO/스토어 운영 최적화(전환율 기준)
- [ ] (스토어) 메타데이터/스크린샷/구독 안내 문구를 “심사 통과” → “전환율” 기준으로 개선
  - 스크린샷 메시지/순서/카피 A/B 계획 수립(가능 범위 내)
- [ ] (스토어) 지원/약관/개인정보 링크 체계 재점검(앱 내/스토어/웹의 일관성)

##### E) Privacy/정책 정합성
- [ ] (공통) 광고/분석(PostHog/AdMob) 및 계정삭제/데이터 보관 정책이 “실제 동작 ↔ 문서”와 일치하는지 점검
...
**다음 단계(현재 기준)**:
1. `10.3.1 iOS 1.0.1 패치 준비` 진행(실기기 회귀 테스트 포함)
2. `10.3.2 결제/리텐션/스토어 운영 고도화` 착수(특히 `code: 8` 재현/분류/UX 정책)
...
- [x] 랜딩 페이지 마케팅 고도화(v1) — 내부 문서/튜토리얼/앱스토어 스크린샷을 활용해 섹션·스크린샷·FAQ·가격 정보를 포함
- [ ] 랜딩 페이지 마케팅 고도화(v2) — 웹서치/레퍼런스 조사 후 카피·구성·스토어 배지·전환 최적화(CTA/소셜프루프) 등 추가 개선
...
- [ ] **자동복원 UX 개선**: App Store 계정 변경/로그인 상황에서 비밀번호/로그인 팝업이 반복 노출되지 않도록 트리거 조건 재설계 (restore는 사용자 명시 액션 위주 + 안내 UX 추가)
...
### B2. 타입 추천 로직 고도화
**목표**: 실천 항목 타입(루틴/미션/참고) 추천 정확도 향상 + 추천 근거/설정 자동추출까지 일관되게 제공
**현재 상태**: 규칙 기반(결정적) 추천 로직 v2가 `@mandaact/shared`에 단일 소스화되어 앱(Web/Mobile)에서 공통 사용

**구현 현황(실제 코드 기준)**:
- [x] 단일 소스: `packages/shared/src/lib/actionTypes.ts` (추천 로직/근거 코드/설정 추출 포함)
- [x] v2 응답: `suggestActionTypeV2()`가 `reason_code`, `extracted_settings`, `debug(input_language 등)`를 제공
- [x] Mobile 적용: `apps/mobile/src/components/ActionTypeSelector.tsx`에서 v2 결과로 분석 이벤트(`action_type_suggested`) 전송
- [ ] AI 기반 분류(선택): `supabase/functions/classify-action-type`는 존재하지만 앱 연동은 아직 없음(실험/백업 옵션)

**작업 목록**:
- [ ] 사용자 피드백 데이터 수집(운영 기반)
  - 추천 노출/수락/거부/수정(수동 변경) 로깅 스키마 정의
  - “추천 → 저장 시 변경 여부” 기반으로 수락/거부 추정 로직 추가
- [ ] 규칙 기반 로직 튜닝(단기)
  - 한국어/영어(혼합) 입력 케이스, 요일/주기/숫자 목표 패턴 보강
  - 저신뢰도 시 자동 적용 대신 안내/선택 UX 유도(적용 기준 재정의)
- [ ] 검증 체계 강화
  - 회귀 방지 테스트 케이스/스냅샷(표준 입력 세트) 추가 및 기준값 문서화
- [ ] (선택) AI 분류 폴백 도입 검토(중기)
  - feature flag 뒤에서만 호출(비용/지연/실패 시 안전 폴백 필수)
  - 운영 데이터로 규칙 기반과 비교 평가 후 도입 여부 결정
