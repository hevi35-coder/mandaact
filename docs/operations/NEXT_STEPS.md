# 다음 단계 - Mobile App 100% 완료 후 남은 작업

**작성일**: 2025-11-27 (Updated)
**현재 상태**: Mobile App 100% 완료 ✅ | Web Phase 8.1 완료 (90%)
**다음 우선순위**: 실제 기기 테스트 → Sentry 활성화 → 앱스토어 배포 준비

---

## 📊 현재 완료 상태

### ✅ Mobile App 100% 완료 (2025-11-27)
...
- ✅ EAS Build 배포 가이드 문서화 (TESTING_GUIDE.md)
- ✅ FadeInUp 애니메이션 (모든 메인 화면)
...
### 0. Mobile App 실제 기기 테스트 (1-2시간) 🔴 **최우선**

**목적**: 49개 테스트 항목 실제 기기 검증

**테스트 가이드**: `docs/mobile/TESTING_GUIDE.md`
...
| 영역 | 항목 | 주요 포인트 |
|------|------|------------|
| 인증 | 6개 | 로그인, 회원가입, 비밀번호 재설정 |
| 홈 | 4개 | XP/레벨, 스트릭, 히트맵 |
| 투데이 | 5개 | 체크박스, 타입 필터 |
| 만다라트 | 18개 | OCR, 편집, 내보내기 |
| 게임화 | 8개 | 배지 21개, 진행률 |
| 설정 | 5개 | 알림 토글, 시간 선택 |

---

### 0.5. Sentry 프로젝트 생성 (10분) 🔴 **추천**

**현재 상태**: 코드 완료, DSN 미설정

**설정 방법**:
1. https://sentry.io 접속 → 로그인
2. Create Project → React Native 선택
3. DSN 복사 (`https://xxx@o123.ingest.sentry.io/456`)
4. `.env` 파일에 추가:
   ```bash
...
**EAS Build (앱스토어 배포 시)**:
```bash
eas secret:create --scope project --name EXPO_PUBLIC_SENTRY_DSN --value "https://xxx@..."
```

---

### 1. PostHog 대시보드 인사이트 생성 (10분) 🟡 **권장**
...
### Mobile App 우선 순서:
1. **실제 기기 테스트** (1-2시간) ← 🔴 **오늘**
2. **Sentry 프로젝트 생성** (10분) ← 🔴 **오늘**
3. **Git Commit & Push** (5분) ← **지금 바로**

### Web App 후속 작업:
4. PostHog 대시보드 인사이트 생성 (10분)
5. Vercel 프로덕션 재배포 (5분)
6. Sentry 알림 규칙 설정 (10분)

### 앱스토어 배포 준비 (나중에):
7. EAS Build 설정 (`eas.json`)
8. EAS Secrets 설정 (SENTRY_DSN)
9. 앱스토어 제출 (iOS App Store, Google Play)
...
| 순위 | 작업 | 예상 시간 | 권장 시점 |
|------|------|----------|----------|
| **1** | Git Commit & Push | 5분 | **지금** |
| **2** | 실제 기기 테스트 (49항목) | 1-2시간 | **오늘** |
| **3** | Sentry 프로젝트 생성 | 10분 | **오늘** |
| 4 | PostHog 대시보드 | 10분 | 이번 주 |
| 5 | Phase 5: UX 디테일 | 1주 | 다음 주 |
| 6 | 앱스토어 배포 | 1-2일 | 테스트 후 |
